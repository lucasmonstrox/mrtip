...
- [x] ui — **spike de deploy** RESOLVIDO (2026-07-20): o gate funciona em produção, mas **exigiu trocar `proxy.ts` por `middleware.ts`**. O `proxy.ts` do Next 16 roda SEMPRE em Node (recusa `runtime: "edge"` no config: _"Proxy always runs on Node.js runtime"_) e o OpenNext/Cloudflare não suporta Node middleware — patcha o `loadNodeMiddleware` do Next pra no-op, falhando o build com _"Node.js middleware is not currently supported"_. O `middleware.ts` legado roda em Edge e funciona. Mesmo padrão de `grupoceralis/apps/crm/middleware.ts`. Provado: `/`, `/jogo`, `/historico` → 307 p/ `/sign-in?redirect_url=…`; `/sign-in`, `/sign-up` → 200.
- [x] api — guard **fail-closed** (2026-07-20): sem `CLERK_SECRET_KEY` o guard só libera sob Bun; no workerd responde 503 `auth_not_configured`. Motivado por incidente real — um deploy sem o secret deixou a API inteira aberta em produção e o guard não avisou.
- [ ] ui — ⏳ **TRAVA DE UPGRADE: não subir pro Next 17 sem antes resolver o middleware.** O `middleware.ts` legado (nosso contorno) deve perder suporte no Next 17, e o `proxy.ts` **nunca** será suportado pelo `@opennextjs/cloudflare` — o mantenedor declarou isso formalmente ([#1213](https://github.com/opennextjs/opennextjs-cloudflare/issues/1213)): _"proxy (formerly Node middleware) will not be supported on this repo. We will support them with the adapters API"_. Uma PR tentando implementar foi fechada com _"This is not a fix [...] it will just crash in prod instead of at build time"_ ([#1280](https://github.com/opennextjs/opennextjs-cloudflare/pull/1280)). O destino é a Adapters API ([adapters-api#20](https://github.com/opennextjs/adapters-api/issues/20), ABERTA). E não há saída pelo lado do Clerk: a doc do `auth()` exige _"clerkMiddleware() to be configured"_ — gatear só em layout/Server Component não é suportado. Acompanhar antes de qualquer upgrade de major do Next.
- [ ] ops — **migrar pra instância de PRODUÇÃO do Clerk quando houver domínio próprio.** Hoje prod roda com as chaves `test` (instância de dev). As `pk_live_`/`sk_live_` existentes estão **inativas**: a instância de produção foi criada apontando pra `joao-galiano-silva.workers.dev` e fica em "Awaiting deployment" pra sempre — o Clerk exige CNAME (`clerk.<dominio>` → `frontend-api.clerk.services`) numa zona que é da Cloudflare, não do dono. Comprovado: `clerk.joao-galiano-silva.workers.dev/v1/environment` → erro Cloudflare 1042, contra HTTP 200 em `clerk.grupoceralis.com.br`. Limites aceitos enquanto isso: 100 usuários e dado de usuário não migra entre instâncias.
