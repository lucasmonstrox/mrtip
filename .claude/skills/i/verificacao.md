...
| **core** | Lógica pura nova (dinheiro/odds/EV, datas, cálculo de calibração) entrou SEM teste ao lado = §3 foi violado — volte e escreva, no app/pacote dono. Não há runner de unidade configurado (nem task `test` no turbo raiz; o Playwright do `apps/web` cobre e2e, não unidade) — sem runner, a prova é o script ad-hoc da linha `api`/`dados`. Dinheiro sempre via `@workspace/core/money` (centavos). |
