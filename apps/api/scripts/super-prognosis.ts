...
const BLOCOS = [
  ...
  "home_gols", "home_estilo", "home_qualidade", "home_forma", "home_momentum", "home_faixas", "home_desfalques",
  "away_gols", "away_estilo", "away_qualidade", "away_forma", "away_momentum", "away_faixas", "away_desfalques",
...
const superSchema = jsonSchema<SuperPrognosis>({
  ...
  required: ["home", "away", "general", "conclusoes", "blocos_sem_sinal", "descartadas", "jogadores", "arestas", "roteiros", "janelas", "tempos", "previsoes_jogadores", "marcadores", "mercados", "best_bet", "refutacao", "drivers"],
  additionalProperties: false,
})
...
function gameSection(r: RunResult): string {
  ...
  <div class="badges">
    ...
    <span class="badge"><b>${cov.conclusoes}</b> conclusões · <b>${cov.arestas}</b> arestas · <b>${cov.jogadores}</b> jogadores</span>
...
<div class="sub">${esc(MODEL)} · thinking on · effort ${esc(EFFORT)} · run ${esc(stamp)} · protocolo: varredura por bloco → jogador a jogador → arestas → roteiros → board de mercados → refutação</div>
...
for (const r of results) {
  ...
  console.log(`  ✓ ${r.homeName} x ${r.awayName} — bet: ${r.output.best_bet.market}/${r.output.best_bet.selection}${r.output.best_bet.line != null ? " " + r.output.best_bet.line : ""} (${r.output.best_bet.confidence}) → ${s ?? "?"} · cobertura ${r.cov!.blocosCobertos + r.cov!.blocosSemSinal}/${BLOCOS.length} blocos, ${r.cov!.arestas} arestas, ${r.cov!.jogadores} jogadores`)
}
