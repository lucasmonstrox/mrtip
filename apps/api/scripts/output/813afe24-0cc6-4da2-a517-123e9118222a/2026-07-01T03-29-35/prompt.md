...
⛳ **RESSALVA ESTRUTURAL (não deixe o veredito acima zerar isto):** o mandante Manchester United MARCOU em **16/18 jogos em casa (89%)** nesta season. Marcar em casa é ESTRUTURAL (qualidade/identidade) e **NÃO** depende de motivação — ele fura mesmo administrando. Portanto **NÃO** banque `under` do JOGO TODO nem `team_total under` dele por inércia de intenção: um eventual under vem de **MENOS gol do VISITANTE**, não do mandante secar. Aqui o mercado de menor variância é **`team_total` over 0.5 (ou 1.5 se os placares dele em casa costumam ser 2+) do MANDANTE** — não o under do jogo.
